...
void render_object(std::shared_ptr<isobus::VTObject> object,
                  ...
                  std::int16_t parentY)
{
    if (!object || !ws || !drawingAPI)
        return;
    
    isobus::CANStackLogger::debug("[Renderer] Rendering object type " + 
                                 std::to_string(static_cast<int>(object->get_object_type())) +
                                 " ID:" + std::to_string(object->get_id()));
    ...
    switch (object->get_object_type())
    {
        ...
        case isobus::VirtualTerminalObjectType::Button:
        {
            ...
            if (buttonObj)
            {
                ...
                for (std::uint16_t i = 0; i < buttonObj->get_number_children(); i++)
                {
                    ...
                    if (childObj)
                    {
                        std::int16_t childX = x + buttonObj->get_child_x(i);
                        std::int16_t childY = y + buttonObj->get_child_y(i);
                        render_object(childObj, ws, drawingAPI, childX, childY);
                    }
                }
            }
            break;
        ...
        case isobus::VirtualTerminalObjectType::AlarmMask:
        {
            for (std::uint16_t i = 0; i < object->get_number_children(); i++)
            {
                ...
                if (childObj)
                {
                    std::int16_t childX = x + object->get_child_x(i);
                    std::int16_t childY = y + object->get_child_y(i);
                    render_object(childObj, ws, drawingAPI, childX, childY);
                }
            }
            break;
        ...
        case isobus::VirtualTerminalObjectType::ObjectPointer:
        {
            ...
            if (ptrObj)
            {
                ...
                if (targetId != isobus::NULL_OBJECT_ID)
                {
                    ...
                    if (targetObj)
                    {
                        render_object(targetObj, ws, drawingAPI, x, y);
                    }
                }
            }
            break;
